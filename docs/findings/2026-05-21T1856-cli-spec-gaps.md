...
status_notes: |
  ...
  2026-06-03 (cycle 260603-01): re-verified against code — this is still the ONLY open item. `change propose`/`change commit` exist (apps/cli/src/commands/change.ts; apps/api/src/http/change-commit-routes.ts appendPreviewRevision) but repurposing them as the conflict-setup path vs adding a dedicated public endpoint is a product decision, not safe unattended. KEEP partial.
related:
  - docs/07-cli-spec.md
