...
- **Application**: merge resolution resolves the merge/source/target, authorizes
  project membership, rejects missing merges or branches, non-open/no-conflict
  merges, stale base revisions, missing manual values, uncovered conflicts, hard
  locks, and simulated write failures without mutating successful state.
- **Application**: merge resolution writes one resolved USECASE revision per
  conflict with injected ids and clock, advances the target branch heads, marks
  the merge request and source branch merged, and returns usecase review
  guidance.
- **Route preservation**: UC-021 E2E and CLI tests continue to prove HTTP
...
### Goal 4 Prisma Split - Membership Store

- **Adapter module**: `tests/unit/prisma-adapter-split.test.ts` imports
  `createPrismaMembershipStore` from
  `src/infrastructure/prisma-membership-store.ts` and asserts it is a function.
- **Setup**: keep the split test as the fast RED/GREEN signal while extracting
  the existing membership persistence methods out of the god Prisma signup
  store.
- **Assertions**: focused adapter-split test, TypeScript build, lint, and the
  full Vitest suite remain green after adding the dedicated adapter.

### Goal 4 Prisma Split - Project Store

- **Adapter module**: `tests/unit/prisma-adapter-split.test.ts` imports
  `createPrismaProjectStore` from
  `src/infrastructure/prisma-project-store.ts` and asserts it is a function.
- **Setup**: keep the split test as the fast RED/GREEN signal while extracting
  the existing project persistence methods out of the god Prisma signup store.
- **Assertions**: focused adapter-split test, TypeScript build, lint, and the
  full Vitest suite remain green after adding the dedicated adapter.
...
### Goal 4 Prisma Split - Branch Store

- **Adapter module**: `tests/unit/prisma-adapter-split.test.ts` imports
  `createPrismaBranchStore` from `src/infrastructure/prisma-branch-store.ts`
  and asserts it is a function.
- **Setup**: keep the split test as the fast RED/GREEN signal while extracting
  the existing branch persistence methods out of the god Prisma signup store.
- **Assertions**: focused adapter-split test, TypeScript build, lint, and the
  full Vitest suite remain green after adding the dedicated adapter.

### Goal 4 Prisma Split - Actor Store

- **Adapter module**: `tests/unit/prisma-adapter-split.test.ts` imports
  `createPrismaActorStore` from `src/infrastructure/prisma-actor-store.ts`
  and asserts it is a function.
- **Setup**: keep the split test as the fast RED/GREEN signal while extracting
  the existing actor persistence methods out of the god Prisma signup store.
- **Assertions**: focused adapter-split test, TypeScript build, lint, and the
  full Vitest suite remain green after adding the dedicated adapter.

### Goal 4 Prisma Split - Stakeholder Store

- **Adapter module**: `tests/unit/prisma-adapter-split.test.ts` imports
  `createPrismaStakeholderStore` from
  `src/infrastructure/prisma-stakeholder-store.ts` and asserts it is a function.
- **Setup**: keep the split test as the fast RED/GREEN signal while extracting
  the existing stakeholder persistence methods out of the god Prisma signup
  store.
- **Assertions**: focused adapter-split test, TypeScript build, lint, and the
  full Vitest suite remain green after adding the dedicated adapter.

### Goal 4 Prisma Split - Stakeholder Interest Store

- **Adapter module**: `tests/unit/prisma-adapter-split.test.ts` imports
  `createPrismaStakeholderInterestStore` from
  `src/infrastructure/prisma-stakeholder-interest-store.ts` and asserts it is a
  function.
- **Setup**: keep the split test as the fast RED/GREEN signal while extracting
  the existing stakeholder-interest persistence methods out of the god Prisma
  signup store.
- **Assertions**: focused adapter-split test, TypeScript build, lint, and the
  full Vitest suite remain green after adding the dedicated adapter.

### Goal 4 Prisma Split - Scenario Store

- **Adapter module**: `tests/unit/prisma-adapter-split.test.ts` imports
  `createPrismaScenarioStore` from
  `src/infrastructure/prisma-scenario-store.ts` and asserts it is a function.
- **Setup**: keep the split test as the fast RED/GREEN signal while extracting
  the existing scenario persistence methods out of the god Prisma signup store.
- **Assertions**: focused adapter-split test, TypeScript build, lint, and the
  full Vitest suite remain green after adding the dedicated adapter.

### Goal 4 Prisma Split - Step Store

- **Adapter module**: `tests/unit/prisma-adapter-split.test.ts` imports
  `createPrismaStepStore` from `src/infrastructure/prisma-step-store.ts` and
  asserts it is a function.
- **Setup**: keep the split test as the fast RED/GREEN signal while extracting
  the existing step persistence methods out of the god Prisma signup store.
- **Assertions**: focused adapter-split test, TypeScript build, lint, and the
  full Vitest suite remain green after adding the dedicated adapter.

### Goal 4 Prisma Split - Goal Store

- **Adapter module**: `tests/unit/prisma-adapter-split.test.ts` imports
  `createPrismaGoalStore` from `src/infrastructure/prisma-goal-store.ts` and
  asserts it is a function.
- **Setup**: keep the split test as the fast RED/GREEN signal while extracting
  the existing goal persistence methods out of the god Prisma signup store.
- **Assertions**: focused adapter-split test, TypeScript build, lint, and the
  full Vitest suite remain green after adding the dedicated adapter.

### Goal 4 Prisma Split - Revision Store

- **Adapter module**: `tests/unit/prisma-adapter-split.test.ts` imports
  `createPrismaRevisionStore` from
  `src/infrastructure/prisma-revision-store.ts` and asserts it is a function.
- **Setup**: keep the split test as the fast RED/GREEN signal while extracting
  the existing revision persistence methods and default-branch context helper
  out of the god Prisma signup store.
- **Assertions**: focused adapter-split test, TypeScript build, lint, and the
  full Vitest suite remain green after adding the dedicated adapter.

### Goal 4 Prisma Split - Work Session Store

- **Adapter module**: `tests/unit/prisma-adapter-split.test.ts` imports
  `createPrismaWorkSessionStore` from
  `src/infrastructure/prisma-work-session-store.ts` and asserts it is a
  function.
- **Setup**: keep the split test as the fast RED/GREEN signal while extracting
  the existing work-session persistence methods out of the god Prisma signup
  store.
- **Assertions**: focused adapter-split test, TypeScript build, lint, and the
  full Vitest suite remain green after adding the dedicated adapter.

### Goal 4 Prisma Split - Merge Request Store

- **Adapter module**: `tests/unit/prisma-adapter-split.test.ts` imports
  `createPrismaMergeRequestStore` from
  `src/infrastructure/prisma-merge-request-store.ts` and asserts it is a
  function.
- **Setup**: keep the split test as the fast RED/GREEN signal while extracting
  the existing merge-request persistence methods out of the god Prisma signup
  store.
- **Assertions**: focused adapter-split test, TypeScript build, lint, and the
  full Vitest suite remain green after adding the dedicated adapter.
...
- **Adapter module**: `tests/unit/prisma-adapter-split.test.ts` imports
  `createPrismaUserStore` from `src/infrastructure/prisma-user-store.ts` and
  asserts it is a function.
- **Setup**: keep the split test as the fast RED/GREEN signal while extracting
  the existing user persistence methods out of the god Prisma signup store.
- **Assertions**: focused adapter-split test, TypeScript build, lint, and the
  full Vitest suite remain green after adding the dedicated adapter.

### Goal 4 CLI Split - Session Command

- **Command module**: `tests/unit/cli-command-split.test.ts` imports
  `SessionCommand` from `src/cli/commands/session.ts` and asserts it extends
  the oclif `Command` base class.
- **Setup**: keep the command-split test as the fast RED/GREEN signal while
  extracting `session start/list/complete`.
- **Assertions**: focused command-split test, TypeScript build, lint, and the
  full Vitest suite remain green after moving the behavior out of
  `src/cli/index.ts`.
...
### Goal 4 CLI Split - Step Command

- **Command module**: `tests/unit/cli-command-split.test.ts` imports
  `StepCommand` from `src/cli/commands/step.ts` and asserts it extends the
  oclif `Command` base class.
- **Setup**: keep the command-split test as the fast RED/GREEN signal while
  extracting `step add/edit`.
- **Assertions**: focused command-split test, TypeScript build, lint, and the
  full Vitest suite remain green after moving the behavior out of
  `src/cli/index.ts`.

### Goal 4 CLI Split - Merge Command

- **Command module**: `tests/unit/cli-command-split.test.ts` imports
  `MergeCommand` from `src/cli/commands/merge.ts` and asserts it extends the
  oclif `Command` base class.
- **Setup**: keep the command-split test as the fast RED/GREEN signal while
  extracting `merge open/resolve`.
- **Assertions**: focused command-split test, TypeScript build, lint, and the
  full Vitest suite remain green after moving the behavior out of
  `src/cli/index.ts`.

### Goal 4 CLI Split - Change Command

- **Command module**: `tests/unit/cli-command-split.test.ts` imports
  `ChangeCommand` from `src/cli/commands/change.ts` and asserts it extends the
  oclif `Command` base class.
- **Setup**: keep the command-split test as the fast RED/GREEN signal while
  extracting `change propose/commit`.
- **Assertions**: focused command-split test, TypeScript build, lint, and the
  full Vitest suite remain green after moving the behavior out of
  `src/cli/index.ts`.

### Goal 4 CLI Split - Sync Command

- **Command module**: `tests/unit/cli-command-split.test.ts` imports
  `SyncCommand` from `src/cli/commands/sync.ts` and asserts it extends the
  oclif `Command` base class.
- **Setup**: keep the command-split test as the fast RED/GREEN signal while
  extracting `pull`, `push`, and `sync`.
- **Assertions**: focused command-split test, TypeScript build, lint, and the
  full Vitest suite remain green after moving the behavior out of
  `src/cli/index.ts`.

### Goal 4 CLI Split - Pull Command

- **Command module**: `tests/unit/cli-command-split.test.ts` imports
  `PullCommand` from `src/cli/commands/pull.ts` and asserts it extends the
  oclif `Command` base class.
- **Setup**: keep the command-split test as the fast RED/GREEN signal while
  adding a dedicated top-level `pull` command that delegates to shared sync
  behavior.
- **Assertions**: focused command-split test, TypeScript build, lint, and the
...
### Goal 4 CLI Split - Lock Command

- **Command module**: `tests/unit/cli-command-split.test.ts` imports
  `LockCommand` from `src/cli/commands/lock.ts` and asserts it extends the
  oclif `Command` base class.
- **Setup**: keep the command-split test as the fast RED/GREEN signal while
  extracting `lock` request/formatting behavior from `src/cli/index.ts`.
- **Assertions**: focused command-split test, TypeScript build, lint, and the
...
### Goal 4 CLI Split - Revert Command

- **Command module**: `tests/unit/cli-command-split.test.ts` imports
  `RevertCommand` from `src/cli/commands/revert.ts` and asserts it extends the
  oclif `Command` base class.
- **Setup**: keep the command-split test as the fast RED/GREEN signal while
  extracting `revert` request/formatting behavior from `src/cli/index.ts`.
- **Assertions**: focused command-split test, TypeScript build, lint, and the
...
- **Command module**: `tests/unit/cli-command-split.test.ts` imports
  `DiffCommand` from `src/cli/commands/diff.ts` and asserts it extends the
  oclif `Command` base class.
- **Setup**: keep the command-split test as the fast RED/GREEN signal while
  extracting `diff` request/formatting behavior from `src/cli/index.ts`.
- **Assertions**: focused command-split test, TypeScript build, lint, and the
...
- **Command module**: `tests/unit/cli-command-split.test.ts` imports
  `ExportCommand` from `src/cli/commands/export.ts` and asserts it extends the
  oclif `Command` base class.
- **Setup**: keep the command-split test as the fast RED/GREEN signal while
  extracting `export gherkin/markdown`.
- **Assertions**: focused command-split test, TypeScript build, lint, and the
  full Vitest suite remain green after moving the behavior out of
  `src/cli/index.ts`.
