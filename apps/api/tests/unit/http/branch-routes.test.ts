...
import { registerBranchRoutes } from "../../../src/http/branch-routes.js";
...
describe("branch routes", () => {
  test("rejects malformed branch create requests", async () => {
    const captured = reply();

    await registeredRoute()(
      request({ body: { from: "main", name: "" } }),
      captured.fastifyReply
    );

    expect(captured.statusCode).toBe(400);
    expect(captured.body).toMatchObject({ title: "Invalid branch request" });
  });
});
...
function registeredRoute(): Handler {
  ...
  if (handler === undefined) {
    throw new Error("expected branch create route");
  }
  return handler;
}
