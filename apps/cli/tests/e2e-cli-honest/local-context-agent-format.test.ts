...
describe("honest CLI local context --format=agent", () => {
  ...
  test("agent local context lifecycle", async () => {
    ...
    const initialStatus = await expectOk(runCli([
      "status",
      "--format=agent"
    ], seed.env));
    ...
    const project = await expectOk(runCli([
      "project",
      "switch",
      "ALT",
      "--format=agent"
    ...
    const statusAfterProject = await expectOk(runCli([
      "status",
      "--format=agent"
    ], seed.env));
    const statusAfterProjectEnvelope = expectAgentEnvelope<StatusData>(statusAfterProject.stdout);
    expect(statusAfterProjectEnvelope.data.config.current_project_key).toBe("ALT");

    const workspace = await expectOk(runCli([
      "workspace",
      "switch",
      "workspace-two",
      "--format=agent"
    ], seed.env));
    const workspaceEnvelope = expectAgentEnvelope<WorkspaceSwitchData>(workspace.stdout);
    ...
    const statusAfterWorkspace = await expectOk(runCli([
      "status",
      "--format=agent"
    ], seed.env));
    const statusAfterWorkspaceEnvelope = expectAgentEnvelope<StatusData>(statusAfterWorkspace.stdout);
    expect(statusAfterWorkspaceEnvelope.data.config.current_workspace_id).toBe("workspace-two");
    expect(statusAfterWorkspaceEnvelope.data.config.current_workspace_slug).toBe("workspace-two");
  });
});
