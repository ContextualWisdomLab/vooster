...
_Auto-generated 2026-05-19T18:30:39Z. Do not hand-edit; use blockers.md for overrides._

```
TASK: Migrate the next entity to Prisma (gates 2.A1 / 2.A2 / 2.A3 / 2.A4).

  Candidate SignupState field: revisionsByEntityId

  One TDD cycle:
  1. Identify the Prisma model that backs revisionsByEntityId. If absent, add it to
  ...
  2. RED: extend tests/integration/persistence-matrix.test.ts with a stanza
     that creates a revisionsByEntityId entity via the HTTP API, restarts the
     server, and reads it back. Commit:
         red(persist): revisionsByEntityId survives restart
  3. GREEN: add src/infrastructure/<entity>-store.ts with Prisma calls,
     ...
     'revisionsByEntityId' field from SignupState in the SAME commit. Commit:
