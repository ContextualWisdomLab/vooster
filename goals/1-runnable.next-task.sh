...
TASK: Make vspec bootable (gate 1.1).
  ...
  - Write a real src/index.ts that imports createServer from src/http/server.ts
    and calls app.listen({ port, host: '0.0.0.0' }).
  - Add a "start" script to package.json: "start": "node dist/index.js".
  - Add a /healthz route returning { status: "ok" } (in src/http/server.ts).
...
if ! bash "$ROOT/scripts/check-persistence.sh" >/dev/null 2>&1; then
  # Find the first src/http/*.ts that still uses in-memory state.
  CANDIDATE=$(grep -lE '(new (Map|Set)\(|Record<[A-Za-z]+,)' src/http/*.ts 2>/dev/null \
    | grep -v -- '-routes.ts$\|server.ts\|signup-types.ts' \
    | head -1)
  if [ -z "$CANDIDATE" ]; then
    CANDIDATE=$(grep -lE 'new (Map|Set)\(' src/http/*.ts 2>/dev/null | head -1)
  fi
  cat <<EOF
TASK: Migrate one route to Prisma persistence (gate 1.2).
  ...
  - Pick the lowest-UC entity that still uses in-memory state. Candidate:
      ${CANDIDATE:-(none detected — search src/http/ for 'new Map(')}
...
TASK: Add the next missing CLI subcommand (gate 1.3).
  ...
  - Add an oclif Command class under src/cli/commands/<topic>/<sub>.ts.
  - It must call the API (use undici / built-in fetch) and render the result.
  - Add a CLI E2E test in tests/e2e-cli/<UC-ID>.test.ts that spawns the binary.
...
if [ "$CLI_COUNT" -lt "$UC_COUNT" ]; then
  ...
  for f in $(ls docs/usecases/UC-*.md 2>/dev/null | sort); do
    ...
    if [ ! -f "tests/e2e-cli/${UC_ID}.test.ts" ]; then
      cat <<EOF
TASK: Add CLI E2E for $UC_ID (gate 1.4).
  - Read: $f
  - Create tests/e2e-cli/${UC_ID}.test.ts that:
