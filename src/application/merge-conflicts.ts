...
export async function useCaseKey(
  ...
  return (
    (await useCaseStore.findUseCaseWithProject(usecaseId))?.usecase.key ?? usecaseId
  );
}
...
async function semanticConflicts(
  ...
  const candidates = touched
    ...
    .map(async (entityId) => {
      ...
      const theirs = await extensionAtRevision(
        revisionStore,
        targetHeads[entityId] ?? ""
      );
...
async function extensionAtRevision(
  ...
  return { condition: condition.join(":"), extension_point: extensionPoint ?? "" };
}
