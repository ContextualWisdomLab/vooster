...
_Auto-generated 2026-05-19T18:01:13Z. Do not hand-edit; use blockers.md for overrides._

```
TASK: Migrate the next entity to Prisma (gates 2.A1 / 2.A2 / 2.A3 / 2.A4).

  Candidate SignupState field: stakeholderInterestsByUseCaseId

  One TDD cycle:
  1. Identify the Prisma model that backs stakeholderInterestsByUseCaseId. If absent, add it to
  ...
  2. RED: extend tests/integration/persistence-matrix.test.ts with a stanza
     that creates a stakeholderInterestsByUseCaseId entity via the HTTP API, restarts the
     server, and reads it back. Commit:
         red(persist): stakeholderInterestsByUseCaseId survives restart
  3. GREEN: add src/infrastructure/<entity>-store.ts with Prisma calls,
     ...
     'stakeholderInterestsByUseCaseId' field from SignupState in the SAME commit. Commit:
