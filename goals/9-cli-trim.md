...
`docs/findings-cli-spec-gaps.md` 는 Goal 7 이 큐잉한 미구현 verb 목록이다.
...
- `docs/findings-cli-spec-gaps.md` 의 *Missing verbs* 섹션에서
...
- **Goal 7 A3/A4/A5** (envelope routing) 은 *agent branch 가 있는 파일*
  ...
  envelope import 까지 강제된다. Goal 9 E1 은 *어떤 파일이 분기를
  가져야 하는가* 를 declared 셋으로 추가 enforce 할 뿐이라 Goal 7 의
  계약을 강화하지 약화하지 않는다.
...
dropped verb 들 (`why`, `examples`, …) 은 *어느 prior gate 에도* gate
...
A3. **Every verb in `DROPPED_VERBS` ∪ `{vspec usecase edit, vspec
    usecase search}` is absent from `docs/findings-cli-spec-gaps.md`
    ...
    9" is fine *as long as the verb token itself is gone from the
    bullet list*; the gate scopes to bullet lines beginning with `-`.

A4. **Every verb in the declared `IN_SCOPE_VERBS` set is absent from
    the `docs/findings-cli-spec-gaps.md` "Missing verbs" section once
...
D1. **Every verb in `IN_SCOPE_VERBS` has a matching honest test
    ...
    either:
      - `<topic>-<action>.test.ts` (e.g. `project-list.test.ts`,
        `usecase-set.test.ts`), or
      - any `<topic>-<group>.test.ts` whose body contains the literal
...
- **No re-introducing dropped verbs.** `why`, `examples`, `explain`,
  ...
  dispatcher. Re-introduction requires a *new* goal with explicit
  rationale.
...
- **No widening Goal 7 `HONEST_UC_SET`.** Goal 9's honest tests cover
  *verbs*, not Cockburn UC scenarios. They land in
...
4. `docs/findings-cli-spec-gaps.md` — the queue being resolved.
...
1. **Tranche A — trim commits.** Remove dropped-verb lines from
   ...
   `docs/findings-cli-spec-gaps.md` to remove both dropped and
...
- Adding any string from `DROPPED_VERBS` back to `docs/07-cli-spec.md`,
  `docs/findings-cli-spec-gaps.md`, `apps/cli/src/index.ts`, or any
