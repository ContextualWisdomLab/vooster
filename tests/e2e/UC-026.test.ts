...
type RevertResponse = {
  ...
  suggested_next_actions: Action[]; usecase: { current_revision_id: string; id: string; title: string };
...
describe("UC-026 - Revert a use case to a previous revision", () => {
  test("MAIN: append a forward revision restoring the target snapshot", async () => {
    ...
    expect(body.revision).toMatchObject({ change_summary: `Revert to ${targetRevision}`,
      entity_id: usecase.id, entity_type: "USECASE", parent_revision_id: currentHead,
      snapshot: { title: "Reviews a refund" }, version_number: 3 });
    expect(body.revision.id).not.toBe(targetRevision);
    expect(body.revision.id).not.toBe(currentHead);
    expect(body.usecase).toMatchObject({ current_revision_id: body.revision.id,
      id: usecase.id, title: "Reviews a refund" });
    expect(body.impact).toEqual({ affected_branches: [], affected_sessions: [], severity: "NON_BREAKING" });
  ...
  test("4a: breaking revert without force returns impact and writes nothing", async () => {
    ...
    const sessionResponse = await startWorkSession(server, setup, {
      agent_type: "CODEX", intent: "Keep refund wording stable", pins: [usecase.key]
    });
    ...
    expect(problem.breaking_changes).toContainEqual({
      path: "usecase.title", revision: currentHead.revision_id, severity: "BREAKING"
    });
    expect(problem.affected_sessions).toEqual([session.id]);
    expect(problem.suggested_next_actions).toContainEqual({ command:
  ...
  test("5a: gherkin drift warning does not block revert", async () => {
    ...
    const response = await revert(usecase.id, setup.cookie, {
      revision_id: targetRevision, simulate_gherkin_drift: true
    });
