...
import { registerRevisionRevertRoutes } from "../../../src/http/revision-revert-routes.js";
...
describe("revision revert routes", () => {
  test("rejects malformed revert requests", async () => {
    const captured = reply();

    await registeredRoute()(request({ revision_id: "" }), captured.fastifyReply);

    expect(captured.statusCode).toBe(400);
    expect(captured.body).toMatchObject({ title: "Invalid revert request" });
  });
});
...
function registeredRoute(): Handler {
  ...
  if (handler === undefined) {
    throw new Error("expected revision revert route");
  }
  return handler;
}
