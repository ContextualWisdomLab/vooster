...
type HistoryResponse = {
  revisions: Array<{ change_summary?: string; version_number: number }>;
};
type ArchiveProblem = {
  archived_at?: string;
...
describe("UC-015 - Archive or restore a use case", () => {
  test("MAIN: archive soft-deletes, writes revision, and hides from default list", async () => {
    ...
    expect(history.revisions.map((revision) => revision.version_number)).toEqual([
      2, 1
    ]);
    expect(history.revisions[0]?.change_summary).toBe(
      `Archived use case ${usecase.key}`
    );
  ...
  test("3b: active hard lock blocks archive without writing a revision", async () => {
    ...
    expect(await revisionHistory(usecase.id, setup.cookie)).toMatchObject({
      revisions: [{ version_number: 1 }]
    });
  });
