...
# .vspec/config.json (api_url + session_token) — the shape the CLI actually
# reads (apps/cli/src/config-store.ts).
#
...
# Unique identity per run so repeated seeds never collide on user/workspace.
UNIQ="dogfood-$(date -u +%Y%m%d%H%M%S)-$$"
CODE="stub-$UNIQ"

hdr_start="$(mktemp)"; body_start="$(mktemp)"
hdr_cb="$(mktemp)"
trap 'rm -f "$hdr_start" "$body_start" "$hdr_cb"' EXIT
...
curl -sS -D "$hdr_start" -o "$body_start" \
  ...
  -d "{\"workspace\":{\"name\":\"Dogfood $UNIQ\",\"slug\":\"$UNIQ\"}}" \
...
# 3. write the config the CLI reads
mkdir -p "$VSPEC_DOGFOOD_REPO/.vspec"
jq -n --arg api "$API" --arg tok "$token" \
  '{api_url:$api, session_token:$tok}' \
  > "$VSPEC_DOGFOOD_REPO/.vspec/config.json"

echo "✓ seeded $VSPEC_DOGFOOD_REPO/.vspec/config.json (api_url=$API, session for $CODE)"
