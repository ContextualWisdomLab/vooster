import type { FastifyInstance, FastifyReply, FastifyRequest } from "fastify";
import { describe, expect, test } from "vitest";
import { registerRevisionHistoryRoutes } from "../../../src/http/revision-history-routes.js";
import type { SignupState } from "../../../src/http/signup-types.js";
import type { MembershipStore } from "../../../src/ports/membership-store.js";
import type { ProjectStore } from "../../../src/ports/project-store.js";
import type { RevisionStore } from "../../../src/ports/revision-store.js";
import type { UseCaseStore } from "../../../src/ports/usecase-store.js";

type Handler = (request: FastifyRequest, reply: FastifyReply) => unknown;

describe("revision history routes", () => {
  test("rejects malformed history requests", async () => {
    const captured = reply();

    await registeredRoute()(request({ limit: "0" }), captured.fastifyReply);

    expect(captured.statusCode).toBe(400);
    expect(captured.body).toMatchObject({ title: "Invalid history request" });
  });
});

function registeredRoute(): Handler {
  let handler: Handler | undefined;
  const app = {
    get: (_path: string, next: Handler) => {
      handler = next;
    }
  } as unknown as FastifyInstance;

  registerRevisionHistoryRoutes(
    app,
    signupState(),
    {} as MembershipStore,
    {} as ProjectStore,
    {} as RevisionStore,
    {} as UseCaseStore
  );

  if (handler === undefined) {
    throw new Error("expected revision history route");
  }
  return handler;
}

function request(query: unknown): FastifyRequest {
  return {
    params: { usecaseId: "usecase-1" },
    query
  } as unknown as FastifyRequest;
}

function reply() {
  const captured: {
    body?: unknown;
    fastifyReply: FastifyReply;
    statusCode?: number;
  } = {
    fastifyReply: undefined as unknown as FastifyReply
  };
  captured.fastifyReply = {
    code: (statusCode: number) => {
      captured.statusCode = statusCode;
      return captured.fastifyReply;
    },
    send: (body: unknown) => {
      captured.body = body;
      return body;
    }
  } as unknown as FastifyReply;
  return captured;
}

function signupState(): SignupState {
  return {
    pendingOAuth: new Map(),
    readOnlyMemberships: new Set(),
    sessionsByToken: new Map()
  };
}
