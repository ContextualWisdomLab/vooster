...
TASK: Add the web terminology label source.
  - In apps/web, add a labels/glossary module for canonical Korean labels.
...
if ! grep -Rqs "TermLabel" apps/web/app apps/web/components apps/web/lib 2>/dev/null; then
  cat <<'MSG'
TASK: Add the glossary label affordance.
  ...
  - Keep it inside apps/web and keep pages read-only.
MSG
  exit 0
fi
